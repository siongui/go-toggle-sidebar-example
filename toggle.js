...
$packages["main"] = (function() {
	var $pkg = {}, $init, js, ptrType, funcType, la, ra, lh, rh, showSidebar, hideSidebar, main;
	js = $packages["github.com/gopherjs/gopherjs/js"];
	ptrType = $ptrType(js.Object);
	funcType = $funcType([ptrType], [], false);
	showSidebar = function(event) {
		var $ptr, event;
		la.classList.toggle($externalize("invisible", $String));
		ra.classList.toggle($externalize("invisible", $String));
		rh.classList.toggle($externalize("invisible", $String));
		lh.style.width = $externalize("50%", $String);
	};
	hideSidebar = function(event) {
		...
		ra.classList.toggle($externalize("invisible", $String));
		rh.classList.toggle($externalize("invisible", $String));
		lh.style.width = $externalize("100%", $String);
	};
	main = function() {
		var $ptr;
		la = $global.document.getElementById($externalize("left-arrow", $String));
		ra = $global.document.getElementById($externalize("right-arrow", $String));
		lh = $global.document.getElementById($externalize("left-half", $String));
		rh = $global.document.getElementById($externalize("right-half", $String));
		la.addEventListener($externalize("click", $String), $externalize(showSidebar, funcType), $externalize(false, $Bool));
		ra.addEventListener($externalize("click", $String), $externalize(hideSidebar, funcType), $externalize(false, $Bool));
	};
	$init = function() {
		$pkg.$init = function() {};
		/* */ var $f, $c = false, $s = 0, $r; if (this !== undefined && this.$blk !== undefined) { $f = this; $c = true; $s = $f.$s; $r = $f.$r; } s: while (true) { switch ($s) { case 0:
